...
export class AuthController {
    constructor(private readonly authService: AuthService,
                ...
                @Post('register')
                public async register(@Response() res, @Body() user: RegisterUserDto){
                
                const duplicateUser = await this.userService.findOne(user);
                console.log("Duplicate user -> ", duplicateUser);
                if(duplicateUser){
                return res.status(HttpStatus.BAD_REQUEST).json("User already Exist");
                }
